...
{.text-center} Une transformation qui _retourne_ une forme est appelée une [__symétrie axiale__](gloss:reflection) . _{.small}(Dans d'autres pays francophones ou en anglais, on utilise le terme « réflexion ».)_
...
Une [__symétrie axiale__](gloss:reflection) est une transformation qui « retourne » une forme par rapport à une droite. Cette droite est appelée l'__axe de symétrie__.
...
Notez que si un point se trouve sur l'axe de symétrie, il [[ne bouge pas|tourne|bascule]] lorsqu'on applique la symétrie : _{span.reveal(when="blank-0")} son image est le même point que l'original._

---
> id: reflections-3

Dans tous les exemples ci-dessus, l'axe de symétrie était horizontal, vertical ou à 45°, ce qui facilitait la construction. Si ce n'est pas le cas, la construction nécessite un peu plus de travail :
...
{.r} Pour construire le symétrique de cette forme par rapport à l'[axe de symétrie](target:refl), nous devons construire le symétrique de chaque [sommet](gloss:polygon-vertex) individuellement, puis les relier. _{button.next-step} Continuer_

{.r.reveal(when="next-0")} Choisissons l'un des sommets et traçons la droite passant par ce sommet qui est perpendiculaire à l'axe de symétrie. _{button.next-step} Continuer_

{.r.reveal(when="next-1")} Maintenant, nous pouvons mesurer la [distance](target:d1) entre le sommet et l'axe de symétrie, et placer le point symétrique à la [même distance](target:d2) de l'autre côté. _{span.lgrey} (Nous pouvons utiliser une règle ou un [compas](target:circ).)_ _{button.next-step} Continuer_
...
{.r.reveal(when="next-3")} Il ne nous reste plus qu'à relier les sommets symétriques dans le bon ordre, et nous avons construit le symétrique de la figure !
...
{.r} Comme pour les symétries axiales, nous devons faire pivoter chaque point d'une forme individuellement. _{button.next-step} Continuer_
...
{.text-center} Nous pouvons appliquer une symétrie axiale à ce papillon, et il a la même apparence après. Nous disons qu'il a __une symétrie axiale__.
...
Une forme a __une symétrie axiale__ si elle a la même apparence après avoir appliqué une symétrie axiale. La droite de symétrie est appelée l'[__axe de symétrie__](gloss:axis-of-symmetry), et elle divise la forme en deux moitiés [[congruentes|égales|semblables]]. Certaines figures peuvent avoir plusieurs axes de symétrie.
...
De nombreuses lettres de l'alphabet ont une symétrie axiale. Sélectionnez toutes celles qui en ont une :
...
Voici d'autres formes. Complétez-les pour qu'elles aient une symétrie axiale :
...
Si nous ignorons les espaces et la ponctuation, les phrases courtes ci-dessous ont également une symétrie axiale. Pouvez-vous créer la vôtre ?
...
* Ajouter deux rotations donnera toujours [[une rotation|une symétrie axiale]] (ou l'identité). * L'ajout de deux symétries axiales donnera toujours [[une rotation|une symétrie axiale]] (ou l'identité). * L'ajout des deux mêmes symétries dans l'ordre opposé [[donne parfois un résultat différent|donne toujours un autre|donne toujours le même]] résultat. * L'ajout de l'identité [[ne fait rien|renvoie une symétrie axiale|renvoie le contraire]].
...
Vous vous êtes peut-être déjà rendu compte que l'ajout __{.orange} symétries__ est en fait très similaire à l'ajout __{.green} entiers__ :
    ...
    ol.proof
      ...
      li.r.reveal(when="next-1")
        | Chaque #[strong.orange symétrie]/#[strong.green entier] a un #[strong inverse], une autre #[strong.orange symétrie]/#[strong.green entier] qui, lorsqu'on l'ajoute, donne l'identité :
        .text-center.axiom
          img.cube(src="images/cube-1.svg" width=32 height=32)
          mo +
          img.cube(src="images/cube-3.svg" width=32 height=32)
...
Dans les [sections précédentes,](/course/transformations/symmetry) nous avons vu deux types de symétrie différents correspondant à deux transformations différentes : les rotations et les symétries axiales. Mais il y a aussi une symétrie pour le troisième type de transformation rigide : les [[translations|rotations|symétries axiales]].
...
En plus de la symétrie axiale, rotationnelle et translationnelle, il existe même un quatrième type : la [__symétrie glissée__](gloss:glide-reflection). Il s'agit d'une combinaison d'une symétrie axiale et d'une translation parallèle à l'axe de symétrie.
...
Il s'avère que, bien qu'il existe une infinité de motifs possibles, ils ont tous l'un des 17 groupes de symétrie différents. Ceux-ci sont appelés les __groupes de papier peint__. Chaque groupe de papier peint est défini par une combinaison de translations, rotations, symétries axiales et symétries glissées. Pouvez-vous voir les [centres de rotation](gloss:center-of-rotation) et les [axes de symétrie](gloss:axis-of-symmetry) dans ces exemples ?
...
En plus des translations, des symétries axiales, des rotations et des symétries glissées, ces groupes incluent des symétries comme __des plans de glissement__ et __des axes hélicoïdaux__ (pensez au mouvement lorsque vous dévissez une bouteille).
...
Par exemple, notre expérience nous dit que les lois de la physique sont les mêmes partout dans l'univers. Peu importe que vous meniez une expérience à Londres, à New York ou sur Mars - les lois de la physique devraient toujours être les mêmes. D'une certaine manière, elles ont une [[symétrie translationnelle|symétrie axiale]].

{.reveal(when="blank-0")} De même, peu importe que nous menions une expérience face au Nord, au Sud, à l'Est ou à l'Ouest : les lois de la nature ont [[une symétrie de rotation|une symétrie glissée]].
...
Par exemple, la symétrie temporelle implique que l' __énergie__ doit être conservée dans notre univers: vous pouvez convertir l'énergie d'un type à un autre (par exemple la lumière en électricité), mais vous ne pouvez jamais créer ou détruire d'énergie. La quantité totale d'énergie dans l'univers restera toujours constante.
    ...
    figure
      ...
      p.caption Le CERN est le plus grand accélérateur de particules au monde. Les scientifiques font entrer en collision des particules fondamentales à des vitesses énormes pour en apprendre davantage sur leurs propriétés. Voyez-vous la personne en bas, pour comparer les tailles ?

::: column(width=220)

    x-img(src="images/higgs.png" width=220 height=150 alt="Particle Fragments")
    p.caption Les trajectoires des fragments de particules après une collision
...
Jusqu'à présent, nous venons de regarder les transformations [[rigides|conformes|visuelles]]. _{span.reveal(when="blank-0")} Maintenant, réfléchissons à une autre : une [__homothétie__](gloss:dilation) change la taille d'une forme en l'agrandissant ou en la réduisant._
...
Toutes les homothéties ont un [__centre__](target:center) et un [__rapport__](->.scale-target). Le centre est le point de référence pour l'homothétie et le rapport nous indique dans quelle mesure la figure s'agrandit ou se réduit.
...
{.todo} À VENIR - Plus d'informations sur les homothéties

...
Les polygones réguliers peuvent avoir de nombreuses tailles différentes - mais tous les polygones réguliers avec le même nombre de côtés [[sont semblables|sont congrus|ont la même aire]] !
...
Tout d'abord, nous pouvons diviser le polygone en ${toWord(n)} triangles congrus, [[isocèles|équilatéraux|rectangles]].

{.reveal(when="blank-0")} Nous connaissons déjà la [[base|hauteur|aire]] de ces triangles, mais nous avons également besoin de la [[hauteur|base|médiane]] pour pouvoir calculer leur aire. _{span.reveal(when="blank-2")} Dans les polygones réguliers, cette hauteur est parfois appelée [{.pill.yellow} apothème](target:apothem)._
...
::: column.grow
    ...
    x-picker.list
      .item.md Les côtés opposés sont [congrus](gloss:congruent).
      .item(data-error="parall-error-1") Les angles internes sont toujours inférieurs à 90°.
      .item.md(data-error="parall-error-2") Les diagonales [coupent en deux](gloss:angle-bisector) les angles internes.
      .item Les angles opposés sont congrus.
      .item(data-error="parall-error-3") Les deux diagonales sont congrues.
      .item(data-error="parall-error-4") Les côtés adjacents ont la même longueur.
      .item Les deux diagonales se coupent en leur milieu.
...
Dans les années 1970, le mathématicien et physicien britannique [Roger Penrose a](bio:penrose) découvert des [pavages](bio:penrose) _non périodiques_ - ils continuent toujours infiniment dans toutes les directions, mais _ne se_ ressemblent _jamais_ exactement. Ce sont des __pavages Penrose__ , et vous n'avez besoin que de quelques types de polygones différents pour en créer un:
    ...
    figure
      ...
      p.caption Déplacez le curseur pour révéler la structure sous-jacente de ce pavage. Remarquez comment les mêmes motifs apparaissent à différentes échelles : les petits pentagones jaunes, les étoiles bleues, les losanges oranges et les « navires » verts apparaissent dans leur taille originale, dans une #[strong.blue taille légèrement plus grande] et dans une #[strong.red taille encore plus grande]. Cette #[em auto-similarité] peut être utilisée pour prouver que ce pavage de Penrose est non périodique.

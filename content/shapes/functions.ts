...
import {isOneOf, repeat, tabulate, tabulate2D} from '@mathigon/core';
...
import {Arc, Circle, intersections, isCircle, isLineLike, Line, Point, Polygon, Polyline, Rectangle, Segment} from '@mathigon/euclid';
...
// Dessine la spirale finale dans l'intro
export function spiraleIntro($step: Step) {
  const $svg = $step.$('svg.spiral-final') as SVGParentView;
  if (!$svg) return;
  ...
  // Dessiner les rayons (lignes vers le centre)
  ...
  // Dessiner la courbe de la spirale
  let pathD = '';
  for (let i = 0; i < numPoints; i++) {
    const r = baseR * (i + 1);
    const angle = (-90 + i * angleStep) * Math.PI / 180;
  ...
  $N('path', {
    d: pathD, fill: 'none', stroke: '#0f82f2', 'stroke-width': 2
  }, $svg);
  ...
  for (let i = 0; i < numPoints; i++) {
    ...
    $N('circle', {
      cx: x, cy: y, r: 2, fill: '#333'
    }, $svg);
  }

  // Point central O
  $N('circle', {cx: cx, cy: cy, r: 3, fill: '#0f82f2'}, $svg);
  $N('text', {x: cx + 8, y: cy + 4, 'font-size': 12, fill: '#0f82f2'}, $svg).text = 'O';

  // Point A
  const aX = cx, aY = cy + baseR;
  $N('circle', {cx: aX, cy: aY, r: 3, fill: '#22ab24'}, $svg);
  $N('text', {x: aX - 12, y: aY + 4, 'font-size': 12, fill: '#22ab24'}, $svg).text = 'A';
}

// Construction interactive avec slider
export function spiraleConstruction($step: Step) {
  const $geopad = $step.$('x-geopad') as Geopad;
  const $slider = $step.$('x-slider') as Slider;
  const $label = $step.$('#step-label');

  if (!$geopad || !$slider) return;

  const svg = $geopad.$svg as SVGParentView;
  const cx = 250, cy = 200;
  const baseR = 22;
  const angleStep = 30; // 30° pour une spirale visible

  const labels = ['A', 'B', 'C', 'D', 'E', 'F', 'G', 'H', 'I', 'J', 'K', 'L', 'M', 'N', 'O', 'P'];
  const colors = ['#22ab24', '#fd8c00', '#cd0e66', '#0f82f2', '#8b5cf6', '#22ab24', '#fd8c00', '#cd0e66'];

  // Éléments SVG qu'on va mettre à jour
  ...
  function drawStep(step: number) {
    ...
    // Point central O (toujours visible)
    elements.push($N('circle', {cx: cx, cy: cy, r: 5, fill: '#0f82f2'}, svg));
    elements.push($N('text', {x: cx - 15, y: cy + 5, 'font-size': 14, fill: '#0f82f2', 'font-weight': 'bold'}, svg));
    elements[elements.length - 1].text = 'O';
    ...
    // Calculer et dessiner les points
    ...
    // Dessiner les rayons (segments O vers chaque point)
    points.forEach((p, i) => {
      elements.push($N('line', {
        x1: cx, y1: cy, x2: p.x, y2: p.y,
        stroke: colors[i % colors.length], 'stroke-width': 1.5, 'stroke-dasharray': '5,3'
      }, svg));
    });

    // Dessiner l'arc de compas pour le dernier point ajouté (si pas le premier)
    if (step > 1) {
      const lastR = baseR * step;
      const prevAngle = (-90 + (step - 2) * angleStep) * Math.PI / 180;
      const currAngle = (-90 + (step - 1) * angleStep) * Math.PI / 180;

      // Arc de compas (cercle partiel)
      const arcStartX = cx + lastR * Math.cos(prevAngle);
      const arcStartY = cy + lastR * Math.sin(prevAngle);
      const arcEndX = cx + lastR * Math.cos(currAngle);
      const arcEndY = cy + lastR * Math.sin(currAngle);

      // Créer l'arc avec SVG path
      const largeArc = Math.abs(currAngle - prevAngle) > Math.PI ? 1 : 0;
      const arcPath = `M ${arcStartX} ${arcStartY} A ${lastR} ${lastR} 0 ${largeArc} 1 ${arcEndX} ${arcEndY}`;

      elements.push($N('path', {
        d: arcPath, fill: 'none',
        stroke: colors[(step - 1) % colors.length],
        'stroke-width': 2, 'stroke-dasharray': '3,2'
      }, svg));
    }

    // Dessiner la spirale (segments entre points consécutifs)
    if (points.length > 1) {
      for (let i = 0; i < points.length - 1; i++) {
        elements.push($N('line', {
          x1: points[i].x, y1: points[i].y,
          x2: points[i + 1].x, y2: points[i + 1].y,
          stroke: '#0f82f2', 'stroke-width': 3
        }, svg));
      }
    }
    ...
    points.forEach((p, i) => {
      elements.push($N('circle', {
        cx: p.x, cy: p.y, r: 6, fill: colors[i % colors.length]
      }, svg));

      // Label du point
      const labelAngle = (-90 + i * angleStep) * Math.PI / 180;
      const labelR = baseR * (i + 1) + 15;
      const labelX = cx + labelR * Math.cos(labelAngle);
      const labelY = cy + labelR * Math.sin(labelAngle);

      elements.push($N('text', {
        x: labelX - 5, y: labelY + 5,
        'font-size': 12, fill: colors[i % colors.length], 'font-weight': 'bold'
      }, svg));
      elements[elements.length - 1].text = labels[i];

      // Afficher la distance (n × r)
      if (i < 4) {
        const distLabel = `${i + 1}r`;
        const midX = (cx + p.x) / 2;
        const midY = (cy + p.y) / 2;
        elements.push($N('text', {
          x: midX + 5, y: midY - 5,
        ...
        elements[elements.length - 1].text = distLabel;
      }
    });

    if ($label) $label.text = String(step);
  }
  ...
  $slider.on('move', (value: number) => {
    drawStep(value + 1);
  });

  // Marquer comme complété quand l'utilisateur atteint la fin
  $slider.on('move', (value: number) => {
    if (value >= 14) $step.score('slider-complete');
  });
}

// Construction interactive de la spirale d'Archimède (pattern SSS)
export async function spiralePratique($step: Step) {
  const $geopad = $step.$('x-geopad') as Geopad;
  if (!$geopad) return;
  ...
  // Fonction pour obtenir la longueur en unités (comme dans SSS)
  const getLength = (x: number) => Math.round(x / 40);
  model.getLength = getLength;

  // Centre O fixe
  const cx = 250, cy = 200;
  const pointO = $geopad.drawPoint(`point(${cx},${cy})`, {name: 'O', color: 'blue', interactive: false});

  // ÉTAPE 1: L'élève trace le segment OA (définit la longueur de base)
  $geopad.switchTool('line');
  $geopad.showGesture(`point(${cx},${cy})`, `point(${cx},${cy - 50})`);

  const segOA = await $geopad.waitForPath<Segment>(
    (p: Path) => isLineLike(p) && p.p1.distanceTo(new Point(cx, cy)) < 15,
    {
      onBegin: (p: GeoPath) => p.setLabel(`\${getLength(${p.name}.length)||''}`, undefined, `${p.name}.midpoint`),
      onIncorrect: () => $step.addHint('incorrect')
    }
  );

  $step.score('segment-oa');
  segOA.$el.addClass('green');
  const [pO, pA] = segOA.components;
  pO.lock();
  pA.lock();

  // Récupérer la longueur OA et l'angle initial
  const baseLength = segOA.value!.length;
  const initialAngle = Math.atan2(
    segOA.value!.p2.y - segOA.value!.p1.y,
    segOA.value!.p2.x - segOA.value!.p1.x
  );

  // ÉTAPE 2: L'élève trace un cercle de rayon 2×OA depuis O
  $geopad.switchTool('circle');
  $geopad.showGesture(pO.name, `point(${cx + baseLength * 2},${cy})`);

  const circle1 = await $geopad.waitForPath<Circle>(
    (p: Path) => isCircle(p) && isOneOf(p.c, pO.value) && nearlyEquals(p.r, baseLength * 2, 15),
    {
  ...
  $step.score('circle-1');
  circle1.$el.addClass('thin light');
  circle1.components[1].$el.setAttr('hidden', 'hidden');

  // ÉTAPE 3: L'élève trace une demi-droite depuis O (définit l'angle)
  $geopad.switchTool('line');

  const ray1 = await $geopad.waitForPath<Segment>(
    (p: Path) => isLineLike(p) && (p.p1.distanceTo(pO.value!) < 15 || p.p2.distanceTo(pO.value!) < 15),
    drawingOptions
  );

  $step.score('ray-1');
  ray1.$el.addClass('orange thin');

  // Calculer l'angle de cette demi-droite
  const ray1Angle = Math.atan2(
    ray1.value!.p2.y - ray1.value!.p1.y,
    ray1.value!.p2.x - ray1.value!.p1.x
  );
  const angleStep = ray1Angle - initialAngle;

  // Point B à l'intersection du cercle et de la demi-droite
  const pB = new Point(
    cx + baseLength * 2 * Math.cos(ray1Angle),
    cy + baseLength * 2 * Math.sin(ray1Angle)
  );
  $geopad.drawPoint(() => pB, {name: 'B', color: 'orange', interactive: false});

  // ÉTAPE 4: L'élève trace un cercle de rayon 3×OA depuis O
  $geopad.switchTool('circle');
  $geopad.showGesture(pO.name, `point(${cx + baseLength * 3},${cy})`);

  const circle2 = await $geopad.waitForPath<Circle>(
    (p: Path) => isCircle(p) && isOneOf(p.c, pO.value) && nearlyEquals(p.r, baseLength * 3, 20),
    {
      onBegin: (p: GeoPath) => p.setLabel(`\${getLength(${p.name}.r)||''}`, undefined, `${p.name}.c.shift(0,-${p.name}.r/2)`),
      onIncorrect: () => $step.addHint('incorrect')
    }
  ...
  $step.score('circle-2');
  circle2.$el.addClass('thin light');
  circle2.components[1].$el.setAttr('hidden', 'hidden');

  // ÉTAPE 5: L'élève trace une autre demi-droite (même angle)
  $geopad.switchTool('line');

  const ray2 = await $geopad.waitForPath<Segment>(
    (p: Path) => isLineLike(p) && (p.p1.distanceTo(pO.value!) < 15 || p.p2.distanceTo(pO.value!) < 15),
    drawingOptions
  );

  $step.score('ray-2');
  ray2.$el.addClass('red thin');

  // Point C
  const angle2 = initialAngle + 2 * angleStep;
  const pC = new Point(
    cx + baseLength * 3 * Math.cos(angle2),
    cy + baseLength * 3 * Math.sin(angle2)
  );
  $geopad.drawPoint(() => pC, {name: 'C', color: 'red', interactive: false});

  // ÉTAPE 6: Cercle de rayon 4×OA
  $geopad.switchTool('circle');

  const circle3 = await $geopad.waitForPath<Circle>(
    (p: Path) => isCircle(p) && isOneOf(p.c, pO.value) && nearlyEquals(p.r, baseLength * 4, 25),
    {
      onBegin: (p: GeoPath) => p.setLabel(`\${getLength(${p.name}.r)||''}`, undefined, `${p.name}.c.shift(0,-${p.name}.r/2)`),
      onIncorrect: () => $step.addHint('incorrect')
    }
  ...
  $step.score('circle-3');
  circle3.$el.addClass('thin light');
  circle3.components[1].$el.setAttr('hidden', 'hidden');

  // ÉTAPE 7: Dernière demi-droite
  $geopad.switchTool('line');

  const ray3 = await $geopad.waitForPath<Segment>(
    (p: Path) => isLineLike(p) && (p.p1.distanceTo(pO.value!) < 15 || p.p2.distanceTo(pO.value!) < 15),
    drawingOptions
  );

  $step.score('ray-3');
  ray3.$el.addClass('purple thin');

  // Point D
  const angle3 = initialAngle + 3 * angleStep;
  const pD = new Point(
    cx + baseLength * 4 * Math.cos(angle3),
    cy + baseLength * 4 * Math.sin(angle3)
  );
  $geopad.drawPoint(() => pD, {name: 'D', color: 'purple', interactive: false});

  // FINAL: Tracer la spirale en reliant A-B-C-D
  $geopad.switchTool('move');

  // Tracer la spirale avec animation
  const spiralPath = $geopad.drawPath(`polyline(${pA.name},B,C,D)`, {
    name: 'spiral',
    classes: 'blue thick',
    animated: 2000
  });

  await $geopad.animateConstruction('spiral', 2000);
